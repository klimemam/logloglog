...
- **筋トレは種目ベースで管理** — ベンチプレス・スクワットなど一般的な種目カタログ(部位別)から選んで、**セットごとに重量×回数を個別記録**(ピラミッドセット対応)。各セットに前回の記録が表示され、±ボタン(重量2.5kg刻み)で素早く調整。セット間の**休憩タイマー**(1:00/1:30/2:00、終了時バイブ)つき。成長は種目ごとの**推定1RM**(Epley式)グラフで確認(自重種目は最大回数)
