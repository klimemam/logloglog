...
- **筋トレは種目ベースで管理** — ベンチプレス・スクワットなど一般的な種目カタログ(部位別)から選んで **セット×回数×重量** を記録。前回の記録がプリフィルされるので入力は数タップ。種目ごとの週間ベスト重量(自重種目は回数)の成長グラフつき
...
```jsonc
{
  ...
  "entries": [
    ...
    { "id": "…", "habitId": "…", "date": "2026-07-12", "time": "07:30",
      "exercise": "ベンチプレス", "sets": 3, "reps": 8, "weight": 60, "value": 3 }
  ]
}
```
