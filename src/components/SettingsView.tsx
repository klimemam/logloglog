...
function LanguageSection() {
  ...
  return (
    <>
      <h2 className="section-title">{t('言語')} / Language</h2>
      <div className="card">
...
function AppInfoSection() {
  ...
  return (
    <>
      <h2 className="section-title">{t('アプリ情報')}</h2>
...
function SyncSection() {
  ...
  return (
    <>
      <h2 className="section-title">{t('マルチデバイス同期')}</h2>
        ...
        {cfg ? (
          <>
            ...
            <button
              ...
              onClick={() => {
                if (confirm(t('同期を解除しますか?(この端末のデータは残ります)'))) setSyncConfig(null)
              }}
            >
              {t('同期を解除')}
            </button>
...
function TransferSection() {
  ...
  return (
    <>
      <h2 className="section-title">{t('引き継ぎコード(アカウント不要)')}</h2>
      <div className="card" style={{ display: 'grid', gap: 8 }}>
  ...
  const importData = (file: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        if (parsed.version === 1 && Array.isArray(parsed.habits) && Array.isArray(parsed.entries)) {
          if (confirm(t('現在のデータをインポート内容で置き換えます。よろしいですか?'))) {
            dispatch({ type: 'import', data: parsed })
          }
        } else {
          alert(t('ファイル形式が正しくありません'))
        }
      } catch {
        alert(t('ファイルを読み込めませんでした'))
      }
    }
  ...
  return (
    <>
      <header className="app-header">
        <h1>{t('設定')}</h1>
        <div className="date">{t('同期・データ管理・共有')}</div>
      </header>
      <main className="app-main">
        ...
        <h2 className="section-title">{t('友達に教える')}</h2>
        <div className="card" style={{ display: 'grid', gap: 8 }}>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
{t('このアプリは登録不要・無料で誰でも使えます。リンクを送るだけでOK。相手の記録は相手の端末だけに保存され、あなたのデータとは完全に別です。')}
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (navigator.share) {
                ...
                alert(t('リンクをコピーしました。友達に貼り付けて送ってください!'))
              }
            }}
          >
            {t('アプリのリンクを共有')}
        ...
        <h2 className="section-title">{t('データ')}</h2>
        <div className="card" style={{ display: 'grid', gap: 8 }}>
